...
class neuralNetwork:
    def __init__(self, inputNodes, hiddenNodes, outputNodes, learningRate, wih,who) -> None:
...
dataweight = open('modelNNReadLetter.txt', 'r')
weights = dataweight.readlines()
wih = np.reshape(np.asfarray(weights[0].split(',')[0:-1]), (200, 784))
who = np.reshape(np.asfarray(weights[1].split(',')[0:-1]), (10, 200))
dataweight.close()


n = neuralNetwork(input_nodes, hidden_nodes, output_nodes, learning_rate, wih, who)
